...
def plot_training_curve():
    ...
    try:
        # 加载训练历史数据
        training_history = np.load('../data/training_history.npy')
        ...
        print("未找到训练历史文件 ../data/training_history.npy")

def simple_analysis(graph_embeddings, structural_scores):
    """简化分析"""
    print("=== 简化分析 ===")
    ...
    print(f"嵌入均值: {graph_embeddings.mean():.6f}")
    print(f"嵌入标准差: {graph_embeddings.std():.6f}")
    ...
    plt.tight_layout()
    plt.savefig('../outputs/simple_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    simple_analysis(graph_embeddings, structural_scores)
