...
class EdgeEmbeddingModel(nn.Module):
    ...
    def forward(self, data: Data) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        前向传播 - 为每个子图输出整个图的embedding
        
        Args:
            data: PyTorch Geometric Data对象，包含节点特征、边特征、边索引等
            
        Returns:
            graph_embedding: 整个图的嵌入向量 [1, embedding_dim]
            structural_score: 图的结构重要性得分 [1]
        """
        ...
        # 第六步：图级别聚合 - 将所有边嵌入聚合为图嵌入
        # 使用平均池化 + 最大池化的组合
        mean_pooling = torch.mean(edge_embeddings, dim=0, keepdim=True)  # [1, embedding_dim]
        max_pooling = torch.max(edge_embeddings, dim=0, keepdim=True)[0]  # [1, embedding_dim]
        
        # 组合池化结果
        combined_pooling = torch.cat([mean_pooling, max_pooling], dim=1)  # [1, 2*embedding_dim]
        
        # 通过图聚合层得到最终图嵌入
        graph_embedding = self.graph_pooling(combined_pooling)  # [1, embedding_dim]
        
        # 第七步：计算图的结构重要性得分
        structural_score = self.structural_scorer(graph_embedding).squeeze(-1)  # [1]
        
        return graph_embedding, structural_score
    
    def get_graph_embeddings(self, data: Data) -> torch.Tensor:
        """获取图嵌入（用于下游任务）"""
        with torch.no_grad():
            graph_embeddings, _ = self.forward(data)
...
class GraphAutoEncoderLoss(nn.Module):
    ...
    def forward(self, graph_embedding: torch.Tensor, 
        ...
        Args:
            graph_embedding: 图嵌入向量 [1, embedding_dim]
            edge_index: 边索引 [2, num_edges]
            structural_score: 图结构重要性得分 [1]
        ...
        # 1. 结构一致性损失
        # 确保结构得分与嵌入向量的模长相关
        embedding_magnitude = torch.norm(graph_embedding, dim=1)
        structural_consistency_loss = F.mse_loss(
            structural_score, embedding_magnitude
        )
        
        # 2. 嵌入正则化损失（防止过拟合）
        embedding_regularization_loss = torch.norm(graph_embedding, dim=1).mean()
        
        # 3. 图结构重建损失 - 基于图嵌入重建边连接
        # 这里可以添加更复杂的图重建损失，比如链接预测
        graph_reconstruction_loss = torch.tensor(0.0, device=graph_embedding.device)
