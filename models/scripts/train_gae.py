...
def train_gae(config: Dict, trial_mode: bool = False) -> Dict[str, Any]:
    """
    训练GAE模型
    ...
    Args:
        config: 配置字典
    """
    logger, log_path = setup_logger("gae", enable_file=not trial_mode)
    ...
    def display_epoch_progress(epoch_idx: int,
                               ...
                               val_metrics_value: Dict[str, float]) -> None:
        bar_len = 30
        ...
        val_loss_text = _fmt(val_metrics_value.get('loss'))
        precision_text = _fmt(val_metrics_value.get('precision'))
        accuracy_text = _fmt(val_metrics_value.get('accuracy'))
        auc_text = _fmt(val_metrics_value.get('auc'))

        base_msg += (f" | Val Loss: {val_loss_text} "
    ...
    for epoch in range(config['training']['num_epochs']):
        ...
        display_epoch_progress(epoch, config['training']['num_epochs'], avg_train_loss, val_metrics)
