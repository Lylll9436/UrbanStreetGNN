...
def train_model(train_graphs, test_graphs, config, num_epochs=100, learning_rate=1e-3, device='cpu'):
    """训练模型"""
    print("创建模型...")
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            # 前向传播
            graph_embedding, structural_score = model(batch)
            
            # 计算图自编码器损失
            loss = gae_loss(graph_embedding, batch.edge_index, structural_score)
        ...
        if avg_test_loss < best_loss:
            ...
            torch.save(model.state_dict(), '../gnn_models/best_edge_embedding_model.pt')
    ...
    # 保存训练历史
    np.save('../data/training_history.npy', np.array(training_history))
...
def evaluate_model_performance(model, graphs, device='cpu'):
    ...
    with torch.no_grad():
        for graph in graphs:
            ...
            graph_embedding, structural_score = model(graph)
            
            all_graph_embeddings.append(graph_embedding.cpu())
            all_structural_scores.append(structural_score.cpu())
    
    # 合并结果 - 现在每个子图输出一个图级别的embedding
    all_graph_embeddings = torch.cat(all_graph_embeddings, dim=0)  # [num_graphs, embedding_dim]
    all_structural_scores = torch.cat(all_structural_scores, dim=0)  # [num_graphs]
    
    # 计算统计信息
    stats = {
        'total_graphs': len(all_graph_embeddings),
        'embedding_dim': all_graph_embeddings.shape[1],
    ...
    print(f"输出形状: {all_graph_embeddings.shape}")  # 应该是 [100, 128]
    ...
    return all_graph_embeddings, all_structural_scores, stats


def save_results(model, graph_embeddings, structural_scores, stats, config):
    """保存结果"""
    # 保存模型
    torch.save(model.state_dict(), '../gnn_models/edge_embedding_model.pt')
    ...
    print("结果已保存！")
    print(f"图嵌入形状: {graph_embeddings.shape}")  # 应该是 [100, 128]
...
def main():
    ...
    # 5. 评估模型
    print("\n📈 步骤5: 评估模型")
    graph_embeddings, structural_scores, stats = evaluate_model_performance(
        model, test_graphs, device
    )
    
    # 6. 保存结果
    print("\n💾 步骤6: 保存结果")
    save_results(model, graph_embeddings, structural_scores, stats, config)
    
    print(f"\n🎉 训练完成！结束时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("\n生成的文件:")
    print("- ../gnn_models/edge_embedding_model.pt (最终模型)")
    print("- ../gnn_models/best_edge_embedding_model.pt (最佳模型)")
    print("- ../data/edge_embedding_results.pt (图嵌入结果)")
    print("- ../config/model_config.json (模型配置)")
    print("- ../data/training_history.npy (训练历史)")
